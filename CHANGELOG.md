...
### 对话能力
- [x] **自定义场景**：预设提示词（名称 + 指令文本）胶囊一键激活，会话级注入；侧边栏内增删改管理，预置 4 个（翻译/总结要点/代码解读/润色改写）
- [x] **上下文/场景智能注入**：同一组选段与同一场景一轮会话只注入一次，追问不重复携带；内容变化自动重新注入
...
- [x] **执行日志**：`logs/host.log` 记录完整 prompt/耗时/退出码，2MB 自动轮转，`CLAUDE_LOG=0` 可关
...
**新增**
- **↗ 存本地**：会话总结 + 原始对话写入本地 `notes/*.md`（文件名 `YYYYMMDD-HHMMSS-标题.md`）
- host 新增 `save` 消息分支与 `runSave()`：`claude -p --resume` 生成总结（纯文本，不需工具权限），host 拼好笔记并落盘，回帧 `{type:"saved",path}`
- background 改为按 type 透传，`ask` / `save` 共用同一条 `claude-stream` 流式通道
- 降级保护：总结失败仍写盘并在 `## 总结` 段标注，保证点击不丢数据；旧版 CLI 无 `--resume` 时把原文内联进总结 prompt

### v1.5.1 - 2026-06-12

**新增**
- **host 执行日志**：默认写入仓库 `logs/host.log`，记录每次提问的完整 prompt、耗时、退出码与 CLI 报错（`[ask]` / `[prompt]` / `[claude stderr]` / `[done]` / `[error]` 五类事件，ISO 时间戳前缀）
- 自动清理：host 启动时检查超过 2MB 即轮转为 `host.log.old`（仅保留一份历史，磁盘占用上限 ≈ 4MB），无需定时任务
- 开关：包装器（host.bat / host.sh）设 `CLAUDE_LOG=0` 整体关闭；日志含 prompt 原文（选中的页面内容），目录/文件以 700/600 权限创建仅本用户可读
- 可靠性：日志任何失败（目录不可写、磁盘异常）静默降级为仅 stderr，不影响问答主流程；面板关闭时先 flush 日志再退出，`[done]` 行不丢失

### v1.5.0 - 2026-06-12

**新增**
- **自定义场景**：场景 = 名称 + 指令文本；输入框上方胶囊条点选激活/再点取消；⚙ 管理视图支持新建/编辑/删除（名称与提示词必填校验，提示词上限 2000 字）
- 场景会话级注入：一轮会话只注入一次（省 token）；中途切换场景或编辑提示词后，下一条消息自动重新注入；「新对话」保留激活场景并在新会话重新注入
- 预置 4 个场景：翻译 / 总结要点 / 代码解读 / 润色改写（首次播种一次，可改可删，删后不复活）
- prompt 分层：场景指令以「【场景要求】」置于最前；场景激活且未输入问题时兜底指令为「请按上述场景要求处理」；scenario 为空时 prompt 与旧版逐字节一致

**调整**
- **选段上下文同样改为会话级一次注入**：同一组选段一轮会话只发送一次，追问不再重复携带（此前每条消息都重复拼入全部选段）；选段增/删/替换后自动重新注入；空问题且上下文无变化时拦截发送
- user 气泡仅在实际注入时显示 🎭 场景 / 📎 上下文 标记
- 场景库持久存 `chrome.storage.local`；增删改采用读-改-写，防多窗口并发覆盖
